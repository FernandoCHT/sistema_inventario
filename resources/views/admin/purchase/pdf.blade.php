...
<body>
        ...
        <div>
            <table id="datos">
                ...
                <tbody>
                    <tr>
                        <th>
                            <p id="proveedor">Nombre: {{$purchase->provider->nombre}}<br>
                                <!--  {{$purchase->provider->document_type}}-COMPRA: {{$purchase->provider->document_number}}<br> -->
                                Dirección: {{$purchase->provider->direccion}}<br>
                                Teléfono: {{$purchase->provider->telefono}}<br>
        ...
        <div>
            <table id="facproducto">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$purchaseDetail->cantidad}}</td>
                        <td>{{$purchaseDetail->product->nombre}}</td>
                        <td>$/ {{$purchaseDetail->precio}}</td>
                        <td>$/ {{number_format($purchaseDetail->cantidad*$purchaseDetail->precio,2)}}</td>
                    </tr>
                    @endforeach
                </tbody>
                <tfoot>
    ...
    <footer>
        ...
        <div id="datos">
            <p id="encabezado">
                {{-- <b>{{$company->nombre}}</b><br>{{$company->description}}<br>Telefono:{{$company->teletelefono}}<br>Email:{{$company->email}} --}}
            </p>
        </div>
    </footer>
</body>
